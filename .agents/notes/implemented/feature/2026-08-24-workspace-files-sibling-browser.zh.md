...
- **让文件列举／读取走 `ctx.fs`。** 拒绝：`ctx.fs` 是面向模型工具的存储 seam（policy 事件、按会话可换的沙箱后端）；骑上去会把 GUI 浏览耦合进模型的限制后端，而且 `ctx.fs` 以 `FsTarget` 为键的 API 并不天然适配任意路径的 GUI 请求。`apiproxy` 在此变更之前从未依赖过 `ctx.fs`，现在也不开始依赖。
...
- wire 新增 `workspace.listEntries`／`workspace.readFile`、一个新的 `file-too-large` 错误码（`path`、`maxBytes` 细节），以及贯穿 `dsh-host-apiproxy/api` → `dsh-client-connection/client` → `dsh-api-remotes/client` → `dsh-client-runtime/client` 的 `WorkspaceEntry`／`WorkspaceEntryListing`／`WorkspaceFileContent` 类型；fixture（`dsh-client-connection` 的 `FixtureApiClient`）提供一棵小型确定性文件树（文本、Markdown、一个二进制／图片），覆盖预览矩阵，供无密钥的组装测试使用。
- `ApiProxyDefaults`／`ApiProxyService.Config` 新增 `workspaceFilesMaxEntries`／`workspaceFilesMaxReadBytes`（按「不在插件里写死可调参数」规则做成部署可配置），二者均为带所述默认值的自然数。
