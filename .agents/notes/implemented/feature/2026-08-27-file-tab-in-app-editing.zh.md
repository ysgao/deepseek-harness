...
**新增 `workspace.writeFile` RPC，采用与它的同级方法完全一样的纯 `node:fs`方式实现，而不是经由 `dsh-fs`。** `workspace.readFile`／`gitStatus`／`gitCommitAll`／`gitDiscardAll` 在 `packages/api/workspace-controller` 中都是用纯 `node:fs/promises` 与 `git` 命令行调用实现的（`files.ts`、`workspace-git.ts`）；`dsh-fs` 的 `FileSystem`／`writeText` 能力只面向模型工具，此前完全没有接入 Workspace Controller 宿主进程。仅为这一次写入就引入它，会为了没有共享收益而跨越一条既有的包边界，因此 `writeWorkspaceFile` 沿用了它的同级方法既有的先例：它绝不会创建新文件（编辑器只会打开此前一次读取已经解析过的文件），并且它的写入是原子的——先写入同目录下的临时文件，再 `rename`——因此一次失败绝不会在目标路径留下一个不完整的文件。
...
- **把 `dsh-fs` 的 `writeText`／`FsWriteIntent` 接入 Workspace Controller 宿主进程**，而不是自建一个 `writeWorkspaceFile`。拒绝：这个进程中没有任何其它方法使用那个能力 seam，仅为一次写入就采用它，就意味着要在一个此前从未需要过它的进程里构造一个 `FileSystem` 后端，跨越一条同级的读取／git 方法特意没有跨越的包边界。
...
- `IWorkspaces` 相应扩宽（`writeFile`／`writeWorkspaceFile`），`WorkspaceFileContent` 的 `text` 分支新增了一个必填的 `version` 字段——每一个实现方（`dsh-api-workspace-controller` 的真实处理器与 `client/` 面，以及 `test-support/client-runtime` 的 `TestWorkspaces` 替身）都新增了匹配的方法，使这些接口在编译期依旧保持完备。
- `WorkspaceErrorDetailsMap` 新增一个错误码 `file-changed`，与 `file-too-large`／`directory-unreadable` 一起成为 `workspace.writeFile` 可能出现的失败之一。
- File 标签的 Edit 模式仅支持文本／Markdown，且在冲突时不提供合并界面——两者都已记录在 `packages/api/workspace-controller/README.md` 与 `packages/client/ui-conversation-files/README.md` 的 Known Limitations 中。
- `files.ts` 新增的 `writeWorkspaceFile`／`hashContent`、`workspace.writeFile` 的 RPC 处理器（依据 `packages/CLAUDE.md` 中面向产品可见插件的非单元测试规则，针对一个真实临时目录工作区做了真实组合测试）、`FileEditor`，以及 `FileView` 的编辑模式接线，都新增了包级别的测试覆盖；这个真实组合测试还覆盖了在强制写入失败情形下原子写入"绝不留下不完整文件"的保证。
