...
**新增第三个 RPC 方法 `workspace.gitStatus({ workspaceId })`，完全照搬 `listEntries`／`readFile` 已有的模板端到端实现。** 它沿用这两者已经在 `WorkspaceController`（`packages/api/workspace-controller/src/index.ts`）上使用的同一套 `@Remote` 方法模式：`types.ts` 中的 `WorkspaceGitStatus` 类型与错误码，一个解析 workspace 后委托给新宿主端原语——`workspace-git.ts` 里的 `workspaceGitStatus`——的 `@Remote('gitStatus')` 方法。与 `listEntries`／`readFile` 不同的是，请求不携带 `path`——git 状态是针对整个仓库一次性扫描的，而不是按浏览到的目录分别扫描（理由见下方"曾考虑的替代方案"）。
...
`TestWorkspaces`（`test-support/client-runtime`）与该包自身的 `WorkspaceRemote` 协议测试替身（`packages/api/workspace-controller/tests/{transport,model}.client.spec.ts`）都实现了这个新方法——记录调用并默认返回 `isRepo: false`，因为这些测试替身都没有建模真实的 git 工作树。
...
- 新增文件：`packages/api/workspace-controller/src/workspace-git.ts`，及其测试 `packages/api/workspace-controller/tests/workspace-git.host.spec.ts`。
...
- 另新增两个 RPC 方法：`workspace.gitCommitAll`，请求为 `{ workspaceId, message }`（空白或仅含空格的信息在 schema 层被拒绝），以及 `workspace.gitDiscardAll`，请求为 `{ workspaceId }`——二者成功时均返回 `void`，且都在非工作树目录下抛出 `git-not-a-repository`／`git-command-failed`（`WorkspaceErrorDetailsMap` 新增条目），而不是像 `gitStatus` 那样返回一个正常结果。
- `IWorkspaces`、`WorkspaceBrowserInjected`、`FilesNodeProps` 均新增 `listWorkspaceGitStatus`，以及 `commitAllWorkspaceChanges`／`discardAllWorkspaceChanges`（在 `FilesNodeProps` 上为 `commitAllChanges`／`discardAllChanges`）；`TestWorkspaces` 与该包自身的 `WorkspaceRemote` 协议测试替身都实现了全部三者，均未建模真实的 git 工作树，因此提交／放弃调用都会走 `gitStatus` 在 `isRepo: false` 时同样使用的 `git-not-a-repository` 错误路径。
