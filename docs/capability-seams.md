...
| `ctx.authorizationController` | `core` | [`api-settings-controller`](../packages/api/settings-controller) | - | - | - | Bridges a registered `ctx.authorization` flow onto the generated Remote namespace: the shared notice/prompt stream and answer routing for a web Sign-in affordance. |
