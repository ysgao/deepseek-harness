...
| `ctx.authorizationController` | `core` | [`api-settings-controller`](../packages/api/settings-controller) | - | - | - | 把已注册的 `ctx.authorization` flow 桥接到生成的 Remote namespace 上：为 Web 端登录入口提供共享的 notice/prompt 流与应答路由。 |
