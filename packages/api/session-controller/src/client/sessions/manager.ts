...
export class SessionManager {
   ...
   * @returns the Host result or a folded transport error.
   */
  async openWorkspacePath(path: string, signal: AbortSignal): Promise<ClientResult<{ opened: true }>> {
    try {
      return toSessionResult(await this.remote.session.openWorkspacePath({ path }, signal))
    } catch (error: unknown) {
      return transportResult(error)
    }
  }
