...
export class ClientSessions implements ISessions {
  ...
  /**
   * Open a Session-resolved path on the Host desktop.
   * @param path - path after best-effort Session workspace resolution.
   * @param signal - cancellation for the native command.
   * @returns confirmation or a business/transport error.
   */
  openWorkspacePath(path: string, signal: AbortSignal): Promise<ClientResult<{ opened: true }>> {
    return this.manager.openWorkspacePath(path, signal)
  }
