...
export class FakeApiClient {
  ...
  onWorkspaceListEntries: (payload: unknown) => Promise<RemoteResult<WorkspaceEntryListing>> =
    () => Promise.resolve(remoteOk({ path: '/', entries: [], truncated: false }))

  onWorkspaceReadFile: (payload: unknown) => Promise<RemoteResult<WorkspaceFileContent>> =
  ...
  onWorkspaceGitPush: (payload: unknown) => Promise<RemoteResult<WorkspaceGitPushValue>> =
    () => Promise.resolve(remoteOk({ pushed: true }))

  onWorkspaceGitFileDiff: (payload: unknown) => Promise<RemoteResult<WorkspaceFileDiff>> =
