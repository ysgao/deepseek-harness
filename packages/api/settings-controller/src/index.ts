 ...
 * `ctx.settings`, owned by the class below; `credentials`; and `authorization`
 * (the web Sign-in affordance's RPC bridge over `ctx.authorization`), each
 * mounted from here as its own plugin.
 *
...
export class SettingsController extends TypertRemoteService {
   ...
   * authorization namespaces beside it. The settings and credentials
   * namespaces stay registered when their provider is absent so calls can
   * return the configuration API's actionable missing-provider diagnostic;
   * authorization has no provider to be absent — `ctx.authorization` is a
   * plain registry, empty until a flow registers.
