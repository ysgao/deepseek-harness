...
export async function writeWorkspaceFile(
  ...
  /* v8 ignore start -- the temp write/rename failing needs a host-permission or
     concurrent-directory-removal condition (chmod semantics differ enough
     across this suite's OS matrix, same rationale as workspace-git.ts's own
     git-command failure guards); not reliably portable to construct. */
  } catch (error: unknown) {
