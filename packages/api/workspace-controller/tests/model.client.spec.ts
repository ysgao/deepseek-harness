...
class FakeWorkspaceRemote implements WorkspaceRemote {
  ...
  onListEntries: (request: WorkspaceListEntriesRequest) => Promise<RemoteResult<WorkspaceEntryListing>> = request =>
    Promise.resolve(remoteOk({ path: request.path, entries: [], truncated: false }))
  onReadFile: (_request: WorkspaceReadFileRequest) => Promise<RemoteResult<WorkspaceFileContent>> = () =>
  ...
  onGitPush: (_request: WorkspaceGitRequest) => Promise<RemoteResult<WorkspaceGitPushValue>> = () =>
    Promise.resolve(remoteOk({ pushed: true }))
  onGitFileDiff: (_request: WorkspaceGitFileDiffRequest) => Promise<RemoteResult<WorkspaceFileDiff>> = () =>
    Promise.resolve(remoteOk({ oldText: null, newText: null }))
  ...
  listEntries(request: WorkspaceListEntriesRequest): Promise<RemoteResult<WorkspaceEntryListing>> {
    this.record('listEntries', request)
    return this.onListEntries(request)
  }

  readFile(request: WorkspaceReadFileRequest): Promise<RemoteResult<WorkspaceFileContent>> {
  ...
  gitCommitAll(request: WorkspaceGitCommitAllRequest): Promise<RemoteResult<WorkspaceGitCommitAllValue>> {
    this.record('gitCommitAll', request)
    return this.onGitCommitAll(request)
  }
  ...
  gitPullRebase(request: WorkspaceGitRequest): Promise<RemoteResult<WorkspaceGitPullRebaseValue>> {
    this.record('gitPullRebase', request)
    return this.onGitPullRebase(request)
  }

  gitPush(request: WorkspaceGitRequest): Promise<RemoteResult<WorkspaceGitPushValue>> {
    this.record('gitPush', request)
    return this.onGitPush(request)
  }

  gitFileDiff(request: WorkspaceGitFileDiffRequest): Promise<RemoteResult<WorkspaceFileDiff>> {
    this.record('gitFileDiff', request)
    return this.onGitFileDiff(request)
  }
...
describe('ClientWorkspaceModel', () => {
  ...
  it('passes every file/git method through to the Remote untouched, since none of them affect the registry projection', async () => {
    const remote = new FakeWorkspaceRemote()
    const model = modelFor(remote)
    const signal = new AbortController().signal
    const listRequest: WorkspaceListEntriesRequest = { workspaceId: wid('w'), path: '/w' }
    await expect(model.listEntries(listRequest, signal)).resolves.toMatchObject({ ok: true, value: { path: '/w' } })
    expect(remote.calls).toContainEqual({ method: 'listEntries', request: listRequest })

    const readRequest: WorkspaceReadFileRequest = { workspaceId: wid('w'), path: '/w/a.txt' }
    await expect(model.readFile(readRequest, signal)).resolves.toMatchObject({ ok: true, value: { kind: 'text' } })
    expect(remote.calls).toContainEqual({ method: 'readFile', request: readRequest })

    const writeRequest: WorkspaceWriteFileRequest = {
      workspaceId: wid('w'), path: '/w/a.txt', content: 'x', expectedVersion: 'v' as WorkspaceFileVersion,
    }
    await expect(model.writeFile(writeRequest, signal)).resolves.toMatchObject({ ok: true, value: { version: 'fake-version' } })
    expect(remote.calls).toContainEqual({ method: 'writeFile', request: writeRequest })

    await expect(model.gitStatus(wid('w'), signal)).resolves.toMatchObject({ ok: true, value: { isRepo: false } })
    expect(remote.calls).toContainEqual({ method: 'gitStatus', request: { workspaceId: 'w' } })

    const commitRequest: WorkspaceGitCommitAllRequest = { workspaceId: wid('w'), message: 'm' }
    await expect(model.gitCommitAll(commitRequest, signal)).resolves.toMatchObject({ ok: true, value: { committed: true } })
    expect(remote.calls).toContainEqual({ method: 'gitCommitAll', request: commitRequest })
    ...
    await expect(model.gitPush(wid('w'), signal)).resolves.toMatchObject({ ok: true, value: { pushed: true } })
    expect(remote.calls).toContainEqual({ method: 'gitPush', request: { workspaceId: 'w' } })

    const diffRequest: WorkspaceGitFileDiffRequest = { workspaceId: wid('w'), path: '/w/a.txt' }
    await expect(model.gitFileDiff(diffRequest, signal)).resolves.toMatchObject({ ok: true, value: { oldText: null } })
    expect(remote.calls).toContainEqual({ method: 'gitFileDiff', request: diffRequest })
  })

  it('folds a thrown file/git Remote call into an internal failure result', async () => {
    const remote = new FakeWorkspaceRemote()
    remote.onListEntries = () => Promise.reject(new Error('transport down'))
    const model = modelFor(remote)
    await expect(model.listEntries({ workspaceId: wid('w'), path: '/w' })).resolves.toMatchObject({
      ok: false, error: { code: 'internal', message: 'transport down' },
    })
  })
})
