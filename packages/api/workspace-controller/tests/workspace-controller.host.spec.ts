...
describe('WorkspaceController Files-tree config', () => {
  it('defaults the listing and read/write byte bounds and rejects negative values', () => {
    expect(WorkspaceController.Config({})).toEqual({
      workspaceFilesMaxEntries: 1000,
      workspaceFilesMaxReadBytes: 20 * 1024 * 1024,
    })
    expect(WorkspaceController.Config({ workspaceFilesMaxEntries: 10, workspaceFilesMaxReadBytes: 2048 }))
      .toEqual({ workspaceFilesMaxEntries: 10, workspaceFilesMaxReadBytes: 2048 })
    expect(() => WorkspaceController.Config({ workspaceFilesMaxEntries: -1 })).toThrow()
    expect(() => WorkspaceController.Config({ workspaceFilesMaxReadBytes: -1 })).toThrow()
  })
})
