...
In `login` mode the runner instead calls `ctx.authorization.begin()` for the given credential key over a terminal `AuthorizationInteraction` (notices print to stdout, each prompt reads one line from stdin, an empty answer declining), then requests exit 0 once the flow's record commits or exit 1 when it is declined or fails.
