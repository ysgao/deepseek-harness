      ...
      inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
        ...
        return {
          ...
          openFile: async (path) => {
            ...
            // The in-app File tab (ui-conversation's optional conversationFileOpener
            // service) takes priority; only fall back to the Host OS-default
            // handoff when that service is absent or declines the session.
            if (ctx.get('conversationFileOpener')?.openFile(sessionId, resolved) === true) return
