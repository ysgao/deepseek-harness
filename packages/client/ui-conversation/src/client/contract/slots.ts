...
export interface ConversationSessionInjected {
  /**
   * Framework-bound sources. `conversationViews` is the package-owned View
   * roster bound only for the Conversation body; `pendingFileOpen` is this
   * session's latest cross-plugin file-open request (the
   * `conversationFileOpener` service's write side) — the component drains it
   * into `conversationStore`'s `openView('file', path)` action and the File
   * view's one-shot owner props.
   */
  readonly hooks: {
