...
describe('Conversation inject API', () => {
  it('assembles the target-neutral read face without Session side effects', async () => {
    ...
    expect(Object.keys(injected)).toEqual(['hooks', 'bindDraftMirror', 'openFile'])
    // Every conversation.view entry, including Chat, Trajectory, and File, is
    // externally provided by its own package and absent from this isolated
    // bench, which mounts only ui-conversation's own apply().
    expect(b.viewSource(ROOT).getSnapshot().map(view => view.id)).toEqual([])
  ...
  it('projects the dynamic View registration ledger', async () => {
    ...
    // Every conversation.view entry — Chat, Trajectory, and File alike — is
    // provided externally by its own package and absent from this isolated
    // bench, which mounts only ui-conversation's own apply().
    ...
    const removeNamed = b.slots.register(
      { name: 'conversation.view', id: 'trajectory', order: 10, label: 'Trajectory' },
      (() => null) as never,
    )
    await vi.waitFor(() => {
    ...
    const removeBare = b.slots.register(
      { name: 'conversation.view', id: 'bare', order: 11 },
      (() => null) as never,
    )
    await vi.waitFor(() => {
