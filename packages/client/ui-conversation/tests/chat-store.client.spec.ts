...
describe('createChatStore', () => {
  ...
  it('actions cover the declared write set', () => {
    ...
    store.actions.setOpenFilePath('/ws/notes.txt')
    expect(store.store.getSnapshot().openFilePath).toBe('/ws/notes.txt')
    store.actions.setOpenFilePath(null)
    expect(store.store.getSnapshot().openFilePath).toBeNull()
  })
