...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: the File tab stays reachable, unlike Chat, once a file has been opened', () => {
    ...
    // (ConversationSession's pendingFileOpen effect, via the injected
    // `openFile` callback); driving the store action directly here, since
    // usePendingFileOpen is stubbed to `undefined` in this harness (see
    // mount()).
    act(() => { b.store.actions.openView('file', '/one/README.md') })
    b.rerender()
