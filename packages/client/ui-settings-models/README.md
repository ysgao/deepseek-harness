...
The API key is not always the only credential path: when `ctx.authorization` (`@deepseek-ai/dsh-authorization`, exposed over the RPC gateway by `dsh-api-settings-controller`'s `authorization` namespace) has a flow registered for a row's derived `CredentialKey` (`<namespace-ns>/<provider>`), the card renders a generic `AuthorizationPanel` beside the API-key field — a "sign in with subscription" affordance for a route whose adapter also offers OAuth (a pi-ai route such as `llm-pi-ai/anthropic` for Claude Pro/Max). The panel carries no per-flow knowledge of its own: it renders `ctx.authorization`'s neutral notice/prompt vocabulary (a message plus an optional url/code; `text`/`secret`/`select` prompts) generically, so it drives any registered flow to completion without a `layout === 'pi-ai'` family check — a route the catalog registers no flow for (DeepSeek, most hand-declared pi-ai routes) shows nothing extra. Closing the card does not cancel a running attempt: state converges by `CredentialKey` across every open tab via the host stream's reconnect replay, so a human who opens the OAuth link in a new tab and closes Settings to wait keeps their attempt running; cancellation is only ever the panel's own explicit button.
