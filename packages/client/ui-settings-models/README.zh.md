...
API 密钥并非唯一的凭据路径：当 `ctx.authorization`（`@deepseek-ai/dsh-authorization`，由 `dsh-host-apiproxy` 通过 RPC 网关暴露）为某一行派生出的 `CredentialKey`（`<namespace-ns>/<provider>`）注册了对应的 flow 时，卡片会在 API 密钥字段旁渲染一个通用的 `AuthorizationPanel`——为其适配器同时提供 OAuth 的路由（例如面向 Claude Pro/Max 的 pi-ai 路由 `llm-pi-ai/anthropic`）提供「订阅登录」入口。该面板本身不掌握任何特定 flow 的知识：它只是通用地渲染 `ctx.authorization` 的中立通知／提示词词汇（一条消息，外加可选的 url/code；`text`/`secret`/`select` 三种提示），因此无需 `layout === 'pi-ai'` 这类家族判断即可把任意已注册的 flow 驱动至完成——catalog 未为某条路由注册任何 flow 时（DeepSeek、绝大多数手工声明的 pi-ai 路由），该路由不会多出任何内容。关闭卡片并不会取消正在进行的尝试：状态会通过 Host 流的重连回放，按 `CredentialKey` 在每个打开的标签页之间收敛，因此有人在新标签页中打开 OAuth 链接后关闭 Settings 等待，其尝试仍会继续运行；取消操作永远只能通过面板自身的显式按钮完成。
