...
async function bench(isLoopback = true, settings?: object, services: object = {}) {
  ...
  const remote = new TestRemote(ctx, {
    ...
    // Emptied at once: the follow stream ends immediately, leaving the runtime idle.
    authorization: {
      list: vi.fn(() => Promise.resolve([])),
      follow: async function* follow() {},
...
describe('pushed invalidations', () => {
  ...
  it('reports a pushed authorization/settled event into ctx.authorization.notifySettled', async () => {
    ...
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const notifySettled = vi.spyOn(b.ctx.authorization, 'notifySettled')
    b.remote.emit('authorization/settled', ['llm-pi-ai/anthropic', 'authorized'])
    expect(notifySettled).toHaveBeenCalledWith('llm-pi-ai/anthropic')
  })
