...
A real Workspace group's grouped-view row set includes one **Files** node (`src/client/files/`), the first row under its header and sibling to its Session rows — the Ungrouped bucket and the flat "In one list" view have no single root path and render none. Expanding it lazily lists the Workspace root through `ctx.workspaces.listEntries` (directories AND files together, unlike the directory-only `-browse` picker above); a directory row expands its own nested level in place, and a file row opens `FileViewer`, an in-app preview dialog that dispatches by extension (`src/client/files/classify.ts`): Markdown through the shared `MarkdownText`, recognized text/code extensions through the line-numbered, shiki-highlighted `ReadBlock`, images through a blob URL built from the read's base64 payload. Every other extension — PDF included — and any read whose decoded content disagrees with its classified kind falls back to one "Open with default app" action wired to `ctx.workspaces.openPath`. The Files node also drives git branch/status display, commit-all/discard-all, fetch, rebase, push, and a side-by-side `HEAD`-vs-working-tree diff, all against the workspace's enclosing git repository through the matching `ctx.workspaces` git methods. The branch name and pending-change count always show for a repository; Pull and Push each show only when the branch's upstream reports a positive behind/ahead commit count (a repository with no configured upstream, or one already in sync, shows neither), and each shows that count next to its icon. Fetch and rebase are deliberately separate steps: the header's Refresh control fetches from the remote (updating the counts and therefore Pull/Push's own visibility) before re-reading status, while Pull itself only rebases onto whatever the last fetch already found — collapse-then-reopen and the other automatic status re-reads stay local-only and never fetch. The tree is read-only for browsing (no create/rename/delete/move of files) and keeps no persisted expansion state: a collapsed-then-reopened level, and a remounted browser, both refetch.
...
- **The Files tree's file-row icon is a placeholder** — `IconFilePlaceholder16` (`dsh-client-ui-primitives`) is not a figma extract like its folder-icon neighbors; no such asset exists yet.
- **No in-app PDF preview** — a `.pdf` file row falls back to `openPath` (the OS default application) like every other unrecognized extension; an in-app viewer is deferred.
- **No search, multi-select, or persistence in the Files tree** — matches the `-browse` workspace-root picker's own posture; a collapsed-then-reopened level and a remounted browser both refetch from the Host.
