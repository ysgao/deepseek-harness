...
真实 Workspace 分组在分组视图下的行集合包含一个 **Files** 节点（`src/client/files/`），是其标题行下方的第一行，与该分组的 Session 行同级——Ungrouped 分组与「单列表」平铺视图没有单一根路径，因此不渲染该节点。展开后会通过 `ctx.workspaces.listEntries` 惰性列出工作区根目录（目录与文件一起返回，不同于上文只列目录的 `-browse` 选择器）；目录行原地展开自己的嵌套层级，文件行则打开 `FileViewer`——一个按扩展名分派的应用内预览对话框（`src/client/files/classify.ts`）：Markdown 走共享的 `MarkdownText`，可识别的文本／代码扩展名走带行号、shiki 高亮的 `ReadBlock`，图片通过由读取结果的 base64 载荷构造的 blob URL 渲染。其余一切扩展名——包括 PDF——以及任何解码内容与分类结果不一致的读取，都会回退到唯一的「用系统默认应用打开」操作，接到 `ctx.workspaces.openPath`。Files 节点同样驱动 git 分支／状态展示、commit-all／discard-all、fetch、rebase、push，以及针对工作区所在 git 仓库的 `HEAD`-与-工作树并排 diff，全部通过 `ctx.workspaces` 上对应的 git 方法完成。分支名与待处理更改数量在存在仓库时始终显示；仅当分支的上游报告落后／领先提交数为正时才显示 Pull／Push（未配置上游或已与上游同步的仓库两者都不显示），且各自在图标旁显示该数量。fetch 与 rebase 是刻意分开的两个步骤：头部的 Refresh 控件会先从远端 fetch（更新计数，从而影响 Pull／Push 自身的可见性），再重新读取状态；而 Pull 本身只会基于上一次 fetch 已经获知的内容做 rebase——折叠再重新展开以及其余自动状态重读都只在本地进行，从不 fetch。该树在浏览层面是只读的（不支持新建／重命名／删除／移动文件），也不持久化展开状态：折叠再重新展开的层级，以及重新挂载的浏览器，都会重新拉取。
