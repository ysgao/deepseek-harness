...
真实 Workspace 分组在分组视图下的行集合包含一个 **Files** 节点（`src/client/files/`），是其标题行下方的第一行，与该分组的 Session 行同级——Ungrouped 分组与「单列表」平铺视图没有单一根路径，因此不渲染该节点。展开后会通过 `ctx.workspaces.listWorkspaceEntries` 惰性列出工作区根目录（目录与文件一起返回，不同于上文只列目录的 `-browse` 选择器）；目录行原地展开自己的嵌套层级，文件行则打开 `FileViewer`——一个按扩展名分派的应用内预览对话框（`src/client/files/classify.ts`）：Markdown 走共享的 `MarkdownText`，可识别的文本／代码扩展名走带行号、shiki 高亮的 `ReadBlock`，图片通过由读取结果的 base64 载荷构造的 blob URL 渲染。其余一切扩展名——包括 PDF——以及任何解码内容与分类结果不一致的读取，都会回退到唯一的「用系统默认应用打开」操作，接到 `ctx.workspaces.openPath`。该树是只读的（没有新建／重命名／删除／移动），也不持久化任何状态：折叠再重新展开的层级，以及重新挂载的浏览器，都会重新拉取。
