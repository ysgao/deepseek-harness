...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    description: 'Host service backing the generated `ctx.remote.authorization` namespace. Every method delegates to `ctx.authorization`, the seam a plugin registers its own flow against; this controller supplies no flows of its own.',
    methods: [
      {
        signature: '@Remote list(): readonly AuthorizationEntry[]',
        description: 'Every registered flow, for a surface listing what can be authorized.',
        parameters: [],
        returns: 'one entry per flow, in registration order.',
      },
      {
        signature: '@Remote({ mode: \'stream\' }) follow(signal: AbortSignal): AsyncIterable<AuthorizationStreamFrame>',
        description: 'Open the shared notice/prompt stream, starting with a baseline of every prompt still pending.',
      ...
      {
        signature: '@Remote respond(key: CredentialKey, answer: string | undefined): void',
        description: 'Answer the prompt pending for a key, from whichever connected configuration page is showing it.',
        parameters: [{ name: 'key', description: 'the credential record whose prompt is being answered.' }, { name: 'answer', description: 'the typed text or chosen option id; omit to decline.' }],
        throws: ['TypertRemoteFailure `authorization-not-found` when no prompt is pending for the key.'],
      },
      {
        signature: '@Remote async begin(key: CredentialKey, method: string | undefined, signal: AbortSignal): Promise<AuthorizationOutcome>',
        description: 'Run one attempt to authorize a key, resolving only once it settles.',
        parameters: [{ name: 'key', description: 'the credential record to authorize.' }, { name: 'method', description: 'which of the flow\'s methods to run; defaults to its first.' }, { name: 'signal', description: 'caller lifetime; abort withdraws the attempt like `cancel`.' }],
        returns: '`authorized` once the flow\'s record is committed, or `cancelled` when declined or withdrawn.',
        throws: ['TypertRemoteFailure `authorization-not-found` when no flow claims the key, `authorization-in-flight` when one is already running, or `authorization-rejected` for any other seam refusal.'],
      },
      {
        signature: '@Remote cancel(key: CredentialKey): void',
        description: 'Withdraw the attempt running for a key, if any (idempotent) — the Cancel button\'s path, distinct from `begin`\'s own `signal` because a request/response transport answers Cancel on a second call, with no handle on the first one\'s signal.',
        parameters: [{ name: 'key', description: 'the credential record whose attempt should stop.' }],
      },
    ],
  },
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: '@Remote(\'listEntries\') listEntries(request: WorkspaceListEntriesRequest, signal: AbortSignal): Promise<WorkspaceEntryListing>',
        description: 'Lists one directory level under a workspace root: subdirectories and regular files together. `path` must be the workspace\'s own canonical path or a descendant of it.',
        parameters: [{ name: 'request', description: 'workspace identity and target directory.' }, { name: 'signal', description: 'caller lifetime; abort stops the scan.' }],
        returns: 'the level\'s entries and truncation flag.',
      },
      {
        signature: '@Remote(\'readFile\') readFile(request: WorkspaceReadFileRequest, signal: AbortSignal): Promise<WorkspaceFileContent>',
        description: 'Reads one regular file under a workspace root for in-app preview.',
        parameters: [{ name: 'request', description: 'workspace identity and target file.' }, { name: 'signal', description: 'caller lifetime; abort stops the read.' }],
        returns: 'the decoded content.',
      },
      {
        signature: '@Remote(\'writeFile\') writeFile(request: WorkspaceWriteFileRequest, signal: AbortSignal): Promise<WorkspaceWriteFileValue>',
        description: 'Overwrites one existing regular file under a workspace root, guarded by `expectedVersion` against a concurrent change.',
        parameters: [{ name: 'request', description: 'workspace identity, target file, new content, and expected version.' }, { name: 'signal', description: 'caller lifetime; abort rejects before publication.' }],
        returns: 'the version the write produced.',
      },
      {
        signature: '@Remote(\'gitStatus\') gitStatus(request: WorkspaceGitRequest, signal: AbortSignal): Promise<WorkspaceGitStatus>',
        description: 'Reports the current branch and pending file changes of the git repository enclosing a workspace\'s own directory.',
        parameters: [{ name: 'request', description: 'workspace identity.' }, { name: 'signal', description: 'caller lifetime; abort rejects with the abort reason.' }],
        returns: 'the workspace\'s git status.',
      },
      {
        signature: '@Remote(\'gitCommitAll\') gitCommitAll(request: WorkspaceGitCommitAllRequest, signal: AbortSignal): Promise<WorkspaceGitCommitAllValue>',
        description: 'Stages every pending change and commits them.',
        parameters: [{ name: 'request', description: 'workspace identity and commit message.' }, { name: 'signal', description: 'caller lifetime; abort rejects with the abort reason.' }],
        returns: 'commit confirmation.',
      },
      {
        signature: '@Remote(\'gitDiscardAll\') gitDiscardAll(request: WorkspaceGitRequest, signal: AbortSignal): Promise<WorkspaceGitDiscardAllValue>',
        description: 'Reverts every tracked file\'s pending change to its `HEAD` content.',
        parameters: [{ name: 'request', description: 'workspace identity.' }, { name: 'signal', description: 'caller lifetime; abort rejects with the abort reason.' }],
        returns: 'discard confirmation.',
      },
      {
        signature: '@Remote(\'gitPullRebase\') gitPullRebase(request: WorkspaceGitRequest, signal: AbortSignal): Promise<WorkspaceGitPullRebaseValue>',
        description: 'Fetches from the remote tracked by the current branch and rebases local commits on top.',
        parameters: [{ name: 'request', description: 'workspace identity.' }, { name: 'signal', description: 'caller lifetime; abort rejects with the abort reason.' }],
        returns: 'pull confirmation.',
      },
      {
        signature: '@Remote(\'gitPush\') gitPush(request: WorkspaceGitRequest, signal: AbortSignal): Promise<WorkspaceGitPushValue>',
        description: 'Pushes the current branch to its configured remote.',
        parameters: [{ name: 'request', description: 'workspace identity.' }, { name: 'signal', description: 'caller lifetime; abort rejects with the abort reason.' }],
        returns: 'push confirmation.',
      },
      {
        signature: '@Remote(\'gitFileDiff\') gitFileDiff(request: WorkspaceGitFileDiffRequest, signal: AbortSignal): Promise<WorkspaceFileDiff>',
        description: 'Reads one file\'s `HEAD` and current working-tree text, for a side-by-side diff.',
        parameters: [{ name: 'request', description: 'workspace identity and target file.' }, { name: 'signal', description: 'caller lifetime; abort rejects with the abort reason.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'AuthorizationStreamFrame',
    declaration: 'export type AuthorizationStreamFrame = {\n    readonly type: \'notice\';\n    readonly key: CredentialKey;\n    readonly notice: AuthorizationNotice;\n} | {\n    readonly type: \'prompt-requested\';\n    readonly key: CredentialKey;\n    readonly prompt: WireAuthorizationPrompt;\n} | {\n    readonly type: \'prompt-resolved\';\n    readonly key: CredentialKey;\n};',
  },
  {
  ...
  {
    name: 'WorkspaceEntry',
    declaration: 'export interface WorkspaceEntry {\n    readonly name: string;\n    readonly path: string;\n    readonly type: \'directory\' | \'file\';\n    readonly hidden: boolean;\n    readonly size?: number;\n}',
  },
  {
    name: 'WorkspaceEntryListing',
    declaration: 'export interface WorkspaceEntryListing {\n    readonly path: string;\n    readonly entries: readonly WorkspaceEntry[];\n    readonly truncated: boolean;\n}',
  },
  {
    name: 'WorkspaceFileContent',
    declaration: 'export type WorkspaceFileContent = {\n    readonly kind: \'text\';\n    readonly content: string;\n    readonly version: WorkspaceFileVersion;\n} | {\n    readonly kind: \'binary\';\n    readonly mediaType: string;\n    readonly data: string;\n};',
  },
  {
    name: 'WorkspaceFileDiff',
    declaration: 'export interface WorkspaceFileDiff {\n    readonly oldText: string | null;\n    readonly newText: string | null;\n}',
  },
  {
  ...
  {
    name: 'WorkspaceGitCommitAllRequest',
    declaration: 'export interface WorkspaceGitCommitAllRequest {\n    readonly workspaceId: WorkspaceId;\n    readonly message: string;\n}',
  },
  {
    name: 'WorkspaceGitCommitAllValue',
    declaration: 'export interface WorkspaceGitCommitAllValue {\n    readonly committed: true;\n}',
  },
  {
    name: 'WorkspaceGitDiscardAllValue',
    declaration: 'export interface WorkspaceGitDiscardAllValue {\n    readonly discarded: true;\n}',
  },
  {
    name: 'WorkspaceGitFileDiffRequest',
    declaration: 'export interface WorkspaceGitFileDiffRequest {\n    readonly workspaceId: WorkspaceId;\n    readonly path: string;\n}',
  },
  {
    name: 'WorkspaceGitPullRebaseValue',
    declaration: 'export interface WorkspaceGitPullRebaseValue {\n    readonly pulled: true;\n}',
  },
  {
    name: 'WorkspaceGitPushValue',
    declaration: 'export interface WorkspaceGitPushValue {\n    readonly pushed: true;\n}',
  },
  {
    name: 'WorkspaceGitRequest',
    declaration: 'export interface WorkspaceGitRequest {\n    readonly workspaceId: WorkspaceId;\n}',
  },
  {
    name: 'WorkspaceGitStatus',
    declaration: 'export interface WorkspaceGitStatus {\n    readonly isRepo: boolean;\n    readonly branch: string | null;\n    readonly files: Readonly<Record<string, string>>;\n}',
  },
  {
  ...
  {
    name: 'WorkspaceListEntriesRequest',
    declaration: 'export interface WorkspaceListEntriesRequest {\n    readonly workspaceId: WorkspaceId;\n    readonly path: string;\n}',
  },
  {
  ...
  {
    name: 'WorkspaceWriteFileRequest',
    declaration: 'export interface WorkspaceWriteFileRequest {\n    readonly workspaceId: WorkspaceId;\n    readonly path: string;\n    readonly content: string;\n    readonly expectedVersion: WorkspaceFileVersion;\n}',
  },
  {
    name: 'WorkspaceWriteFileValue',
    declaration: 'export interface WorkspaceWriteFileValue {\n    readonly version: WorkspaceFileVersion;\n}',
  },
]
