...
`workspace.listEntries`／`workspace.readFile` 服务于 Web GUI 中每个工作区的文件树（与其会话行并列的同级项），独立于 `host.listDirectory` 那个只列目录的工作区根目录选择器（见下文）。`listEntries` 列出工作区根目录下的一层——目录**与**普通文件一起返回，目录在前，各分组内按名称排序，每个分组各自受 `workspaceFilesMaxEntries`（默认 1000）独立限界——在 `src/workspace-files.ts` 中基于 `node:fs/promises` 实现，而非 `ctx.directoryPicker` seam。`readFile` 对合法 UTF-8 返回 `{ kind: 'text', content }`，否则返回 `{ kind: 'binary', mediaType, data: base64 }`，受 `workspaceFilesMaxReadBytes`（默认 20MB，读取前先用 `stat` 探测）限界，超限报 `file-too-large`（细节含 `path`、`maxBytes`）。两者都要求 `path` 是所寻址工作区自身的规范根路径或其文件系统后代（基于 `path.relative` 的包含性判断，因此共享名称前缀的兄弟目录会被拒绝），否则回答 `directory-unreadable`；未知的 `workspaceId` 回答 `workspace-not-found`。
