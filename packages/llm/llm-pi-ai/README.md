...
- **A sign-in lives only in the process that started it** — an authorization attempt is not durable ([`dsh-authorization`'s own limitation](../../credentials/authorization/README.md#known-limitations-and-deferred-work)), so restarting that process abandons it and the human starts over. `dsh --profile headless login` calls `begin()` from the CLI invocation itself, so exiting the command loses it; the web Models page's sign-in calls it from the long-lived host process, so a browser reload or reconnect does not abandon it (the still-pending prompt replays), only an actual host restart does. Signing out is `deleteRecord` on the stored record, which forgets it locally without telling the issuer.
