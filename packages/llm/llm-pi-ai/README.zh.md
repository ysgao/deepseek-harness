...
- **登录只存在于发起它的进程中**——一次授权尝试并不持久（[`dsh-authorization` 自身的限制](../../credentials/authorization/README.zh.md#known-limitations-and-deferred-work)），因此重启该进程会放弃这次尝试，人需要重新开始。`dsh --profile headless login` 是在 CLI 调用本身里调用 `begin()` 的，所以退出该命令就丢失了它；Web 版 Models 页面的登录是从长期存活的宿主进程发起调用的，因此浏览器刷新或重新连接不会放弃它（仍处于 pending 状态的 prompt 会重放），只有宿主进程真正重启才会。退出登录是对已存储记录执行 `deleteRecord`，只在本地忘记它，不会告知签发方。
