...
import type {
  AgentContext, ClientResult, ISessions, ProjectionsFace, SessionBinding, SessionFace, SessionListState,
...
export class TestSessions implements ISessions {
  ...
  readonly calls: {
    method: 'create' | 'open' | 'openSubagent' | 'setSubagentCatalogOpen' | 'refreshSubagents'
      | 'clear' | 'refresh' | 'search' | 'fork' | 'openWorkspacePath'
    args: unknown[]
  }[] = []
  ...
  /**
   * Open a Session-resolved path on the Host desktop (recorded; always succeeds).
   * @param path - path after best-effort Session workspace resolution.
   * @param signal - forwarded like the production face.
   * @returns confirmation.
   */
  openWorkspacePath(path: string, signal: AbortSignal): Promise<ClientResult<{ opened: true }>> {
    this.calls.push({ method: 'openWorkspacePath', args: [path, signal] })
