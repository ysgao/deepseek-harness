...
import type { WorkspaceFileVersion, WorkspaceId } from '@deepseek-ai/dsh-api-workspace-controller/client'
...
describe('sessions', () => {
  ...
  it('records service-face calls and retains catalog addresses only for addressed selection', async () => {
    ...
    await expect(runtime.sessions.openWorkspacePath('/ws/notes.txt', controller.signal))
    ...
    expect(runtime.sessions.calls).toEqual([
      ...
      { method: 'openWorkspacePath', args: ['/ws/notes.txt', controller.signal] },
    ])
    await runtime.dispose()
  })
...
describe('workspaces action face', () => {
  it('records every IWorkspaces verb with inert defaults and honors stubs', async () => {
    ...
    await expect(ws.gitFileDiff('w1' as WorkspaceId, '/proj/file.ts')).resolves.toEqual({ oldText: null, newText: null })
    await expect(ws.listEntries('w1' as WorkspaceId, '/proj')).resolves.toMatchObject({ path: '/proj', entries: [], truncated: false })
    ...
    ws.stub('gitFileDiff', () => Promise.resolve({ oldText: 'old', newText: 'new' } as never))
    ws.stub('listEntries', () => Promise.resolve({ path: '/stubbed', entries: [], truncated: true } as never))
    ws.stub('readFile', () => Promise.resolve({ kind: 'text', content: 'stubbed', version: 'v2' } as never))
    ws.stub('gitStatus', () => Promise.resolve({ isRepo: true, branch: 'main', files: {} } as never))
    ...
    ws.stub('push', push)
    ws.stub('writeFile', () => Promise.resolve('v3' as never))
    ...
    expect(ws.list.getSnapshot().archivedSessionIds).toEqual(['s1'])
    await expect(ws.gitFileDiff('w1' as WorkspaceId, '/proj/file.ts')).resolves.toEqual({ oldText: 'old', newText: 'new' })
    await expect(ws.listEntries('w1' as WorkspaceId, '/proj')).resolves.toMatchObject({ path: '/stubbed', truncated: true })
    await expect(ws.readFile('w1' as WorkspaceId, '/proj/file.ts')).resolves.toMatchObject({ content: 'stubbed' })
    await expect(ws.gitStatus('w1' as WorkspaceId)).resolves.toEqual({ isRepo: true, branch: 'main', files: {} })
    await ws.commitAllChanges('w1' as WorkspaceId, 'another commit')
